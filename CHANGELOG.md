...
### Changed
- `configs/embedding_config.yaml` — параметр `top_k` поднят с `3` до `5`, чтобы конфиг соответствовал BL-01 (issue #91): production-путь Streamlit UI и пайплайн получают одинаковый top_k по умолчанию (UI слайдер уже использовал 5 явно).
