...
### Added
- **MINOR: metadata inheritance & coverage improvement (issue #90).** `knowledge_base/indexing/build_index.py` теперь применяет stateful Section Propagation: чанки без собственного заголовка наследуют `section_number` / `section_title` от ближайшего предыдущего заголовка в том же документе, с hierarchical reset (новый заголовок того же или более высокого уровня сбрасывает контекст) и page-distance safety fallback (по умолчанию `6` страниц, конфигурируется через `section_inheritance.max_page_distance`). Каждый чанк получает аудит-флаг `section_inherited: bool`. Реалистичный MVP-порог покрытия зафиксирован как `metadata_coverage_min: 0.65` в `configs/embedding_config.yaml` (NFR-02 stretch — `0.95`). Подробности и метрики до/после — в `docs/analysis/metadata-coverage-fix_v1.md`; стандарт обновлён до `docs/standards/embedding-model.md` v1.2 §5.3. Регресс-тесты — `tests/test_metadata_extraction.py` (11 новых case'ов: depth, inherit, hierarchical reset, page-distance fallback, document isolation, end-to-end coverage uplift).
