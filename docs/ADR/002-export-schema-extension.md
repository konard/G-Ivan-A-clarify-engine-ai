...
`[Провайдер]`, `[Ошибка]`, диагностические поля) присутствует в коде
`src/exporters/excel_exporter.py`, но **не закреплён как обязательный в MVP** —
его состав и формат подлежат валидации с БА в ходе Пилота.
...
- До принятия ADR-002 экспортер может эмитировать дополнительные колонки
  сверх MVP-четвёрки, но **никакая внешняя интеграция не должна полагаться**
  на их состав или порядок.
