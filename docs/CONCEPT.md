...
**Связанные задачи:** [issue #37](https://github.com/G-Ivan-A/mango-tz-ai-analyzer/issues/37) (v2.0), [issue #43](https://github.com/G-Ivan-A/mango-tz-ai-analyzer/issues/43) (v2.1)
...
Настоящий документ — **единый источник истины (Single Source of Truth)** по проекту `mango-tz-ai-analyzer`. Он фиксирует бизнес-контекст, требования (FR/НФТ), архитектуру, риски и план внедрения ИИ-инструмента для автоматизированного анализа тендерных технических заданий (ТЗ) на этапе первичного скрининга.

### 1.1. Цель проекта
Снизить трудоёмкость и повысить согласованность ручного анализа ТЗ за счёт автоматической классификации каждого атомарного требования в одну из четырёх категорий — `Да` / `Нет` / `Частично` / `НД` (нет данных) — с обязательной генерацией обоснования и цитат на внутреннюю документацию продуктов Mango Office.
...
| Цель использования | Проверка входящих ТЗ и запросов на доработки на соответствие функциональности Mango Office |
...
- В корпус знаний попадает только **публичная документация** Mango Office и **внутренний перечень функциональности**.
...
- Зарубежные LLM-API (Qwen, DeepSeek) допускаются **только** при включённом флаге `use_test_data_mode: true` и после маскирования.
...
| **NFR-04** | Резидентность данных | Production: только RU-резидентные LLM (GigaChat, YandexGPT); зарубежные провайдеры только при `use_test_data_mode: true` | Аудит `configs/llm_config.yaml`, проверка флага `allowed_for_production` | `configs/llm_config.yaml`, [`docs/audit/data-masking_v1.md`](audit/data-masking_v1.md) |
...
| **NFR-08** | Доступность сервиса | ≥ 99 % на этапе Пилот при доступности хотя бы одного из 4 LLM-провайдеров | Healthcheck fallback-цепочки, мониторинг latency и retries | планируется на этапе Пилот |
...
6. **LLM-классификатор** — fallback-цепочка из 4 провайдеров со строгим JSON-выводом (FR-04).
...
[Файл .xlsx/.docx]
   ...
   → FR-04 LLM-классификация (Qwen → DeepSeek → GigaChat → YandexGPT)
...
|-----------|-----------|---------------|----------------------|
| 1 | Qwen (DashScope) | Зарубежная | Только `use_test_data_mode: true` |
| 2 | DeepSeek | Зарубежная | Только `use_test_data_mode: true` |
| 3 | GigaChat | РФ | ✅ |
| 4 | YandexGPT | РФ | ✅ |
...
**Объём работ:**
1. Подключение 2–3 БА (Mango Office) в роли пилотных пользователей.
...
**Объём работ:**
1. 100 % покрытие продуктов Mango Office в KB.
...
5. GPU-нода для `bge-m3` или эмбеддинги-as-a-service (YandexCloud).
...
| **БЗ / KB** | База знаний — корпус документации Mango Office |
...
| 3 | Параллелизация LLM-вызовов | ✅ Закрыт | MVP — **1 пользователь, последовательная обработка** требований одним активным провайдером. Очередь / параллельные запросы — пост-пилот (зависит от TOS GigaChat / YandexGPT). | Раздел 6.4, 8.1.3 |
